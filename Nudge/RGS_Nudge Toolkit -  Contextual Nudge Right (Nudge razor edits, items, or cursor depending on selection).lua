...
local function SelectTracks(tracks)
    ...
    reaper.Main_OnCommand(40914, 0) -- Touch the first track
    ScrollMixerToFirstSelectedTracks()
    reaper.PreventUIRefresh(-1)
...
local function Main()
    ...
    if RazorExists() then
        ...
        nudge =  reaper.GetCursorPosition() - GetFirstRazorEdge()
            ...
            if razor_data.isEnvelope then
                ...
                if nudge_razor_contents_envelopes then
                    ...
                    if reaper.CountAutomationItems(razor_data.envelope) > 0 then
                        ...
                        if ripple_state == 0 then
                            ...
                            for i = 0, reaper.CountAutomationItems(razor_data.envelope) -1 do
                                ...
                                local gap =  item_start - razor_data.areaEnd
                                ...
                                elseif math.abs(item_start - (razor_data.areaStart + nudge)) < .000001 and math.abs(item_end - (razor_data.areaEnd+nudge)) <.000001 then
                                    ...
                                    --table.insert(AIs_to_delete, {envelope = razor_data.envelope , idx = i } )
                                ...
                                elseif math.abs(item_start - (razor_data.areaStart + nudge)) < .000001 and item_end > razor_data.areaEnd+nudge then
                                    ...
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i, "D_LENGTH", item_length + gap - nudge, true)
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i, "D_STARTOFFS", item_offset - gap + nudge, true)
                                ...
                                elseif item_start < razor_data.areaStart + nudge and item_end > razor_data.areaEnd + nudge then
                                    ...
                                    reaper.Main_OnCommand(42087, -1) -- Envelope: Split automation items
                                    reaper.SetEditCurPos(cur_pos_1, false, false)
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i+index_mod, "D_POSITION", razor_data.areaEnd + nudge, true)
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i+index_mod, "D_LENGTH", item_length + gap - nudge, true)
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i+index_mod, "D_STARTOFFS", item_offset - gap + nudge, true)
                                    reaper.GetSetAutomationItemInfo(razor_data.envelope, i+index_mod, "D_UISEL", 0, true)
                                    index_mod = 1     
                    ...
                    if ripple_state == 0 then
                        ...
                        if razor_data.areaStart + nudge > razor_data.areaEnd then
    ...
    if not RazorExists() and include_markers then
        selected_markers = GetSelectedVisibleRegionsOrMarkers()
    ...
    if not RazorExists() and reaper.CountSelectedMediaItems(0) == 0 and not AutoItemSelected() and (not include_markers or #selected_markers == 0) then
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, false, 0)
    end
    reaper.Undo_EndBlock("Contextual Nudge Right", -1)
end
--------Run----------------------------
reaper.PreventUIRefresh(1)
Main()
