...
local function Main()
    if RazorExists() then
        ...
        local nudge = GetFirstRazorEnd() - reaper.GetCursorPosition() 
    ...
    elseif reaper.CountSelectedMediaItems(0) > 0 then
        if AutoItemSelected() then
            ...
            ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, true, 0)
            local nudge = first_item_end -reaper.GetCursorPosition() 
            reaper.SetEditCurPos(initial_cur_pos, false, false)

            if nudge < shortest_item_length and nudge < shortest_auto_item_length  then
                TrimAutoItemsEnd(-nudge)
                ApplyNudgeRGS(0, snap, 3, nudge_unit, nudge_amount, true, 0)
            end
        else
            ApplyNudgeRGS(0, snap, 3, nudge_unit, nudge_amount, true, 0)
        end
    elseif AutoItemSelected() then
        ...
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, true, 0)
        local nudge = first_auto_item_end - reaper.GetCursorPosition()
        reaper.SetEditCurPos(initial_cur_pos, false, false)
