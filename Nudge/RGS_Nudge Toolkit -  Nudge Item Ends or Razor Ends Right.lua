...
local function GetFirstRazorEnd()
    if RazorExists() then
        ...
        for r = 1, #razor_edits do
            ...
            if razor_edit_data.areaStart < first_edge then
                ...
                first_end = razor_edit_data.areaEnd
            end
        end
        return first_end
    end
...
local function GetFirstSelectedAutoItemEnd()
                ...
                if tonumber(env_vis) == 1 then
                    for k = 0, reaper.CountAutomationItems(env) - 1 do
                        if reaper.GetSetAutomationItemInfo(env ,k, "D_UISEL", 0, false) ~=0 then
                            ...
                            local auto_item_end = edge + length
                            if edge < first_edge then
                                first_edge = edge
                                first_end = auto_item_end
                            end
                        end
                    end
                end
...
local function GetFirstSelectedItemEnd()
    ...
    return first_end, first_item
end

local function TrimAutoItemsEnd(trim_amount)
    for i = 0, reaper.CountTracks(0)-1 do
        local track = reaper.GetTrack(0, i)
        for j = 0, reaper.CountTrackEnvelopes(track)-1 do
            local env = reaper.GetTrackEnvelope(track, j)
...
local function Main()
    if RazorExists() then
        ...
        reaper.SetEditCurPos(GetFirstRazorEnd(), false, false)
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, false, 0)
        local nudge = reaper.GetCursorPosition() - GetFirstRazorEnd() 
        ...
        for i = 1, #razors do 
            ...
            if razor_data.isEnvelope then
                SetEnvelopeRazorEdit(razor_data.envelope, razor_data.areaStart, razor_data.areaEnd + nudge, false)
            else
                SetTrackRazorEdit(razor_data.track, razor_data.areaStart,razor_data.areaEnd + nudge, false, razor_data.areaTop, razor_data.areaBottom)
            end
        end

    elseif reaper.CountSelectedMediaItems(0) > 0 then
        if AutoItemSelected() then
            local first_item_end = GetFirstSelectedItemEnd(false)
            local initial_cur_pos = reaper.GetCursorPosition()
            reaper.SetEditCurPos(first_item_end, false, false)
            ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, false, 0)
            local nudge =  reaper.GetCursorPosition() - first_item_end 
            reaper.SetEditCurPos(initial_cur_pos, false, false)
            TrimAutoItemsEnd(nudge)
            ApplyNudgeRGS(0, snap, 3, nudge_unit, nudge_amount, false, 0)
            
        else
            ApplyNudgeRGS(0, snap, 3, nudge_unit, nudge_amount, false, 0)
        end
    elseif AutoItemSelected() then
        local first_auto_item_end = GetFirstSelectedAutoItemEnd()
        local initial_cur_pos = reaper.GetCursorPosition()
        reaper.SetEditCurPos(first_auto_item_end, false, false)
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, false, 0)
        local nudge = reaper.GetCursorPosition() - first_auto_item_end 
        reaper.SetEditCurPos(initial_cur_pos, false, false)
        TrimAutoItemsEnd(nudge)
    end 
end
