...
local function Main()
    ...
    if RazorExists() then
        ...
        local nudge = GetFirstRazorEdge() -reaper.GetCursorPosition()
    ...
    elseif reaper.CountSelectedMediaItems(0) > 0 then
        if AutoItemSelected() then
            local first_item_edge = GetFirstSelectedItemEdge(false)
            local first_auto_item_edge = GetFirstSelectedAutoItemEdge()
            local initial_cur_pos = reaper.GetCursorPosition()
            reaper.SetEditCurPos(first_item_edge, false, false)
            ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, true, 0)
            local nudge = first_item_edge - reaper.GetCursorPosition() 
            reaper.SetEditCurPos(initial_cur_pos, false, false)

            if nudge < first_auto_item_edge and nudge < first_item_edge  then
                TrimAutoItemsStart(-nudge)
                ApplyNudgeRGS(0, snap, 1, nudge_unit, nudge_amount, true, 0)
            end
        else
            ApplyNudgeRGS(0, snap, 1, nudge_unit, nudge_amount, true, 0)
        end
    elseif AutoItemSelected() then
        ...
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, true, 0)
        local nudge = first_auto_item_edge - reaper.GetCursorPosition()
        reaper.SetEditCurPos(initial_cur_pos, false, false)
        TrimAutoItemsStart(-nudge)
    end 
end
