...
local function Main()
    if selection_end > selection_start then
        local selection_start, selection_end = reaper.GetSet_LoopTimeRange(false,true,0,0, false)
        local cur_pos_1 = reaper.GetCursorPosition()
        reaper.SetEditCurPos(selection_start,false,false)
        local cur_pos_2 = reaper.GetCursorPosition()
        ApplyNudgeRGS(0, snap, 6, nudge_unit, nudge_amount, false, 0)
        local nudge = reaper.GetCursorPosition() - cur_pos_2
        reaper.GetSet_LoopTimeRange(true,true,selection_start+nudge,selection_end+nudge, false)
