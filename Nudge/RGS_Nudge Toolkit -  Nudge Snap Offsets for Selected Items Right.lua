...
local function GetShortestSelectedAreaAfterSnapOffset()
    local item_count = reaper.CountSelectedMediaItems(0)
    local length = reaper.GetMediaItemInfo_Value(reaper.GetSelectedMediaItem(0, 0), "D_LENGTH")
    local snap_offset = reaper.GetMediaItemInfo_Value(reaper.GetSelectedMediaItem(0, 0), "D_SNAPOFFSET")
    local shortest_length_after_snap_offset =  length - snap_offset
    if item_count > 1 then
        for i = 1, item_count - 1 do
            local item = reaper.GetSelectedMediaItem(0, i)
            local length= reaper.GetMediaItemInfo_Value(item, "D_LENGTH")
            local snap_offset = reaper.GetMediaItemInfo_Value(item, "D_SNAPOFFSET")
            local length_after_snap_offset = length - snap_offset
            if length_after_snap_offset < shortest_length_after_snap_offset then 
                shortest_length_after_snap_offset = length_after_snap_offset
            end
        end
    end
    return shortest_length_after_snap_offset
end
...
local function Main()
     if reaper.CountSelectedMediaItems(0) > 0 then
        ...
        local shortest_area_after_snap_offset = GetShortestSelectedAreaAfterSnapOffset()
        ...
        local nudge = reaper.GetCursorPosition() - first_snap_offset_time
        reaper.SetEditCurPos(initial_cur_pos, false, false)

        if nudge < shortest_area_after_snap_offset then
