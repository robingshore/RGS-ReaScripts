...
local function Main()
    ...
    if #selected_regions>0 then
        ...
        local first_region_end = reaper.GetRegionOrMarkerInfo_Value(0, selected_regions[1], "D_ENDPOS")
        reaper.SetEditCurPos(first_region_end, false, false)
        reaper.ApplyNudge(0, snap, 6, nudge_unit, nudge_amount, true, 0)
        local nudge = first_region_end - reaper.GetCursorPosition()
        reaper.SetEditCurPos(cur_pos_1, false, false)
        

        -----Check if Selected Regions are all longer than nudge
        for i = 1, #selected_regions do
            ...
            local length = end_position -start_position
        ...
        if not too_small then
            ...
            reaper.Undo_EndBlock("Nudge Visible Selected Regions End Left",0)
        end
    end
end
